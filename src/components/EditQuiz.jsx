...
const questionsArOnChangeHandler = (questionsAr, { action, questionIndex, choiceIndex, value }) => {
    ...
    if (action.toUpperCase() === 'QUESTION_BODY_CHANGE') {
        return questionsAr.map(({ questionBody, ...rest }, qindex) => {
            return {
                ...rest,
                questionBody: qindex === questionIndex ?
                                value :
                                questionBody,
            }
        })
...
const EditQuiz = ({ location: { id } }) => {
    ...
    const updateQuestion = useCallback(async (questionID, questionIndex, questionBody, choices) => {
    ...
    return token ? (
        <div className={'quizWrapper'}>
            ...
            {questionsAr.map(({ questionID, questionBody, choices }, questionIndex) => {
                // console.log("hello testing " + questionIndex)
                return (
                    <div key={questionIndex}>
                        <h1>{questionIndex + 1}</h1>
                        <textarea value={questionBody} onChange={({ target: { value }  }) => {
                            setQuestionsAr(questionsArOnChangeHandler(questionsAr,
                        ...
                        {choices.map(({ choiceBody, isCorrect }, choiceIndex) => {
                            return (
                                <div key={choiceIndex}>
                                    <div>
                                        <input type="radio" checked={isCorrect} onChange={() => {
                                            setQuestionsAr(questionsArOnChangeHandler(questionsAr,
                                                                                {action: 'CHOICE_SELECT_CHANGE',
                                                                                 questionIndex,
                                                                                 choiceIndex}))
                                        }} />
                                        <textarea value={choiceBody} onChange={({ target: { value } }) => {
                                            setQuestionsAr(questionsArOnChangeHandler(questionsAr, {action: 'CHOICE_BODY_CHANGE',
                                                                                     questionIndex,
                                                                                     choiceIndex,
                                                                                     value}))
                                                                                     console.log(questionsAr)
                        ...
                        <button onClick={() => { updateQuestion(questionID, questionIndex, questionBody, choices) }}>Change question</button>
                    </div>
                )
            })}
        </div>
